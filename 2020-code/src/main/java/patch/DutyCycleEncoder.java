// patch to support simulation
// see https://github.com/wpilibsuite/allwpilib/pull/2387
...
/**
 * Class for supporting duty cycle/PWM encoders, such as the US Digital MA3 with PWM Output, the CTRE Mag Encoder, the
 * Rev Hex Encoder, and the AM Mag Encoder.
 */
public class DutyCycleEncoder implements Sendable, AutoCloseable
{
    ...
    public double get()
    {
        ...
        DriverStation.reportWarning("Failed to read Analog Encoder. Potential Speed Overrun. Returning last value",
            false);
     ...
     * <p>
     * getPositionInRotation() - getPositionOffset() will give an encoder absolute position relative to the last reset.
     * This could potentially be negative, which needs to be accounted for.
     *
    ...
    /**
     * Set the distance per rotation of the encoder. This sets the multiplier used to determine the distance driven
     * based on the rotation value from the encoder. Set this value based on the how far the mechanism travels in 1
     * rotation of the encoder, and factor in gearing reductions following the encoder shaft. This distance can be in
     * any units you like, linear or angular.
     *
     ...
     * {@link #setDistancePerRotation(double)}.
     *
     ...
     * <p>
     * This uses the duty cycle frequency to determine if the sensor is connected. By default, a value of 100 Hz is used
     * as the threshold, and this value can be changed with {@link #setConnectedFrequencyThreshold(int)}.
     *
